(function (global, factory) {
  ...
  const CustomDragDrop = ({ lists, onDragEnd, mode = 'reorder', permanentShadows = {}, setPermanentShadows }) => {
      ...
      const [placeholderIndex, setPlaceholderIndex] = React.useState(null);
      const [mousePosition, setMousePosition] = React.useState({ x: 0, y: 0 });
      const [dragPreview, setDragPreview] = React.useState(null);
      const [draggedItemOriginalIndex, setDraggedItemOriginalIndex] = React.useState(null);
      const [itemConnections, setItemConnections] = React.useState({});
      const listRefs = React.useRef({});
      const lastUpdateRef = React.useRef({ listId: null, index: null });
  ...
  })(exports.DragMode || (exports.DragMode = {}));
  ...
  const DragDropProvider = ({ children, onDragEnd, onDragStart, onDragUpdate, mode = exports.DragMode.REORDER, enableDragPreview = true, dragPreviewOptions = {}, }) => {
      ...
      const registerDraggable = React.useCallback((config) => {
          dispatch({ type: 'REGISTER_DRAGGABLE', payload: config });
      }, []);
      const unregisterDraggable = React.useCallback((draggableId) => {
          dispatch({ type: 'UNREGISTER_DRAGGABLE', payload: draggableId });
      }, []);
      const registerDroppable = React.useCallback((droppableId, config) => {
          dispatch({ type: 'REGISTER_DROPPABLE', payload: { id: droppableId, config } });
      }, []);
      const unregisterDroppable = React.useCallback((droppableId) => {
          dispatch({ type: 'UNREGISTER_DROPPABLE', payload: droppableId });
      }, []);
      const getDraggable = React.useCallback((draggableId) => {
          return state.draggables.get(draggableId);
      }, [state.draggables]);
      const getDroppable = React.useCallback((droppableId) => {
          return state.droppables.get(droppableId);
      }, [state.droppables]);
      const setPermanentShadows = React.useCallback((shadows) => {
          dispatch({ type: 'SET_PERMANENT_SHADOWS', payload: shadows });
      }, []);
      const addPermanentShadow = React.useCallback((id, shadow) => {
          dispatch({ type: 'ADD_PERMANENT_SHADOW', payload: { id, shadow } });
      }, []);
      const removePermanentShadow = React.useCallback((id) => {
  ...
  const Draggable = React__default["default"].memo(({ draggableId, index, children, type = 'default', data, isDragDisabled = false, isShadow = false, listId, permanentShadows, }) => {
      const context = useDragDropContext();
      const elementRef = React.useRef(null);
      const [isDragging, setIsDragging] = React__default["default"].useState(false);
      const [dragOffset, setDragOffset] = React__default["default"].useState(null);
  ...
  const Droppable = React__default["default"].memo(({ droppableId, children, type = 'default', isDropDisabled = false, }) => {
      const context = useDragDropContext();
      const elementRef = React.useRef(null);
      const [isDraggingOver, setIsDraggingOver] = React__default["default"].useState(false);
      const [draggingOverWith, setDraggingOverWith] = React__default["default"].useState(undefined);
      const [draggingFromThisWith, setDraggingFromThisWith] = React__default["default"].useState(undefined);
      // Memoized configuration to prevent unnecessary re-registrations
      const config = React.useMemo(() => ({
          type,
          isDropDisabled,
      ...
      // Drop zone detection
      const isPointInDropZone = React.useCallback((point) => {
          const element = elementRef.current;
          if (!element)
              return false;
          const rect = element.getBoundingClientRect();
      ...
      const calculateDropIndex = React.useCallback((point) => {
          const element = elementRef.current;
          if (!element)
              return 0;
          const rect = element.getBoundingClientRect();
      ...
      const handleGlobalMouseMove = React.useCallback((event) => {
      ...
      const handleGlobalMouseUp = React.useCallback((event) => {
  ...
  var DragDropItem$1 = React__default["default"].memo(DragDropItem);
  ...
  var DragDropList$1 = React__default["default"].memo(DragDropList);
  ...
  var DragPreview$1 = React__default["default"].memo(DragPreview);
  ...
  var ShadowItem$1 = React__default["default"].memo(ShadowItem);
  ...
  const useDragDrop = (options) => {
      ...
      const dragRefCallback = React.useCallback((element) => {
          dragRef.current = element;
      }, []);
      const dropRefCallback = React.useCallback((element) => {
      ...
      const dropProps = React.useCallback(() => ({
          style: {
              minHeight: '50px',
  ...
  function useDragPreview() {
      const [state, setState] = React.useState(initialPreviewState);
      const updateMousePosition = React.useCallback((position) => {
          setState(prev => ({
              ...prev,
      ...
      const updatePreviewPosition = React.useCallback((x, y) => {
          setState(prev => ({
              ...prev,
              mousePosition: { x, y },
  ...
  function useDragState() {
      const [state, dispatch] = React.useReducer(dragReducer, initialDragState);
      const lastUpdateRef = React.useRef({ listId: null, index: null });
      const updateTimeoutRef = React.useRef(null);
      const startDrag = React.useCallback((item, sourceList, originalIndex, preview) => {
          dispatch({
              type: 'START_DRAG',
              payload: { item, sourceList, originalIndex, preview },
          });
      }, []);
      const updatePosition = React.useCallback((dragOverList, placeholderIndex) => {
          // Only update if the position actually changed significantly
          const lastUpdate = lastUpdateRef.current;
      ...
      const addConnection = React.useCallback((itemId, connection) => {
          dispatch({
              type: 'ADD_CONNECTION',
  ...
  function useListRefs() {
      const listRefs = React.useRef({});
      const setListRef = React.useCallback((listId, element) => {
          listRefs.current[listId] = element;
      }, []);
      const getListRef = React.useCallback((listId) => {
          return listRefs.current[listId];
      }, []);
      const getAllListRefs = React.useCallback(() => {
          return listRefs.current;
      }, []);
      const clearListRefs = React.useCallback(() => {
          listRefs.current = {};
      }, []);
      return {
  ...
  function usePermanentShadows() {
      const [permanentShadows, setPermanentShadows] = React.useState({});
      const addShadow = React.useCallback((itemId, shadow) => {
          setPermanentShadows(prev => ({
              ...prev,
              [itemId]: shadow,
          }));
      }, []);
      const removeShadow = React.useCallback((itemId) => {
          setPermanentShadows(prev => {
      ...
      const updateShadow = React.useCallback((itemId, updates) => {
          setPermanentShadows(prev => ({
              ...prev,
              [itemId]: { ...prev[itemId], ...updates },
          }));
      }, []);
      const clearShadows = React.useCallback(() => {
          setPermanentShadows({});
      }, []);
      const hasShadow = React.useCallback((itemId) => {
          return !!permanentShadows[itemId];
      }, [permanentShadows]);
      const getShadow = React.useCallback((itemId) => {
          return permanentShadows[itemId];
      }, [permanentShadows]);
      return {
          permanentShadows,
  ...
  exports.CustomDragDrop = CustomDragDrop;
  exports.DragDropItem = DragDropItem$1;
  exports.DragDropList = DragDropList$1;
  exports.DragDropProvider = DragDropProvider;
  exports.DragPreview = DragPreview$1;
  exports.Draggable = Draggable;
  exports.Droppable = Droppable;
  exports.Placeholder = Placeholder$1;
  exports.ShadowItem = ShadowItem$1;
  exports.useDragDrop = useDragDrop;
  exports.useDragPreview = useDragPreview;
  exports.useDragState = useDragState;
  exports.useListRefs = useListRefs;
  exports.usePermanentShadows = usePermanentShadows;
